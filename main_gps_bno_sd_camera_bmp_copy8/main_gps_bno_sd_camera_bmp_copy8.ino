...
//スタック検知
bool isStucked(LocationData data1, LocationData data2);
void avoidStuck();
...
void loop(void)
{
  ...
  time3 = millis(); 
  time4 = millis();
  LocationData previous_data = {1.0, 1.0, 1.0, 1.0};
  ...
  while(1){
    ...
    if(time4 - time3 > 10000){
      ...
      time3 = millis();
      if(isStucked(now_data, previous_data)){
        avoidStuck();
      }
      previous_data = now_data;
...
bool isStucked(LocationData data1, LocationData data2){
  const double tolerance = 0.000001; // 誤差許容範囲
  double latDiff = abs(data1.latitude - data2.latitude);
  double lngDiff = abs(data1.longitude - data2.longitude);
...
void avoidStuck(){
  ...
  while(time6 - time5 <5000){
    time6 = millis();
  }
  time5 = millis();
  time6 = millis();
  motor.forward(0);
  while(time6 - time5 <3000){
    time6 = millis();
  }
  motor.freeze(0);
  return;
